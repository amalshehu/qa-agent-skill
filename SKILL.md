...
5. GitHub: `gh issue create` can't upload images into the body, so host the artifacts first — a dedicated `qa-artifacts` branch or a gist, pushed via `gh` — and embed the raw URLs as `![repro](…)` markdown. Ask the user which hosting they prefer the first time, then reuse that choice.
